    ...
    <Sheet open={isSheetOpen} onOpenChange={(open) => {
      ...
      <SheetContent className="w-full max-w-2xl p-6 mx-auto bg-white rounded-lg shadow-lg overflow-auto">
        ...
        <div className="space-y-4">
          ...
          <div>
            <Label htmlFor="status" className="block text-sm font-medium text-gray-700 mb-0.5">Selecione o ativo</Label>
            <Select
              onValueChange={(e) => {
                const [prefix, id] = e.split('-');
                if (prefix === 'asset') {
                  handleValueChange(id, 'Asset');
                } else if (prefix === 'element') {
                  handleValueChange(id, 'Element');
                }
              }}
            >
              <SelectTrigger className="w-[180px] focus:ring focus:ring-offset-0 focus:ring-blue-500 mt-4">
              ...
              <SelectContent className="block text-sm font-medium text-gray-700">
                <SelectGroup>
                  <SelectLabel className="underline">Ativos</SelectLabel>
                  {assets.map((asset) => (
                    <SelectItem key={asset.id} value={`asset-${asset.id}`}>
                      {asset.assetName}
                    </SelectItem>
                  ))}
                  <SelectLabel className="underline">Elementos</SelectLabel>
                  {element.map((element) => (
                    <SelectItem key={element.id} value={`element-${element.id}`}>
                      {element.elementName}
                    </SelectItem>
                  ))}
                </SelectGroup>
