...
export const FormBuilderEditor: React.FC<FormBuilderEditorProps> = ({ id }) => {

  //Variables
  const { forms,newForm, selectedForm, updateFormFields,fetchforms } = useAppStateEditor(id)
  ...
  //Hooks
  useEffect(() => {
    fetchforms(id , hasCalled , setHasCalled)
  }, [selectedForm , id , hasCalled , setHasCalled , fetchforms]);
    ...
    if (moreInfo.find((s) => s === id)) {
      if (type === "string") {
        ...
        return (
          <TableRow className="border-b">
            <td></td>
            <TableCell style={{ display: "table-cell" }} colSpan={9} className="flex flex-col gap-2" >
              <FormField
                control={form.control}
                name={`fields.${idx}.desc`}
                render={({ field }) => (
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <FormControl>
                              ...
                              >
                                {field.value
                                  ? enumlist.find((item) => item.name === field.value)
                                      ?.name
                                  : "Selecione a lista"}
                                <ChevronDown
                                  size={22}
                                ...
                                {inputs.map((input) => (
                                  ...
                                  >
                                    <Input
                                      ...
                                      value={input.value}
                                      onChange={(e) =>
                                        handleChangeInput(input.id, e.target.value)
                                      }
                                      placeholder=""
                                    />
                                    <Button
